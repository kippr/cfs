...
def amortizing_loan(principal=None, rate=None, years=None,
    ...
    async def amortizing_loan_cfs(sim):
        ...
        for period in range(months):
            ...
            yield sim.cf(amortization_payment, payment_acct, principal_acct, 'Amortization payment for period {period+1}/ {months}')

            interest_payment = int_schedule[period]
            yield sim.cf(interest_payment, payment_acct, interest_acct, 'Interest payment for period {period+1}/ {months}')
